...
for (int i = 0; i < 2; i++)
{
    ...
    Console.WriteLine($"Введите координату Y {i + 1}-й точки:");
...
distance = Math.Sqrt(Math.Pow((coordinateX[0] - coordinateX[1]), 2)
                     + Math.Pow((coordinateY[0] - coordinateY[1]), 2)
                     + Math.Pow((coordinateZ[0] - coordinateZ[1]), 2));
Console.WriteLine("Расстояние между точками с указанными координатами равно {0:F6}", distance);
